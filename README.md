...
It's a state machine, effectively just a spaghetti code mealy state machine. When testing and coding it I treated it as if it is stochastic in nature due to how annoying it is to debug edge cases.
...
-   `src/testmain.py`: Core bot logic, it's highly redundant and unoptimized.
-   `src/press.py`, `src/input.py`: Input simulation helpers that do the same thing lol you refactor it.
